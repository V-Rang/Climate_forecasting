...
class Model(nn.Module):
    ...
    def forward(self, input_arr, input_enc_arr):
        ...
        bsize, seq_len, height, width, num_vars = input_arr.shape #(b,s,h,w,v)
        input_arr = input_arr.view((input_arr.shape[0], input_arr.shape[1], int(input_arr.shape[2] * input_arr.shape[3]), input_arr.shape[4])) #(b,s,l,v)
        input_arr = input_arr.permute(0,2,3,1) # b,s,l,v -> b, l, v, s
        ...
        # labels = ClusterDetermine(input_arr.view((input_arr.shape[0], input_arr.shape[1], int(input_arr.shape[2] * input_arr.shape[3]))) ) #(b, lat * lon)
        ...
        dec_out = dec_out.view((dec_out.shape[0], height, width, num_vars, self.pred_len)) #(b,l,v,p) -> (b,h,w,v,p)
        dec_out = dec_out.permute(0,4,1,2,3) #(b,h,w,v,p) -> (b,p,h,w,v)
        return dec_out
