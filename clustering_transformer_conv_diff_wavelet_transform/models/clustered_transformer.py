...
class Model(nn.Module):
    ...
    def forward(self, input_arr, input_enc_arr):
        ...
        bsize, seq_len, height, width = input_arr.shape
        ...
        if self.wavelet_transform:
            dec_out_wv = dec_out.clone().detach().cpu().numpy()
            dec_out_wv = self.wave_rec(dec_out_wv, height, width) #(b,p,l) -> (b,p,h*w)
            
            with torch.no_grad():
                dec_out.copy_(torch.tensor(dec_out_wv,device = self.device))

        dec_out = dec_out.reshape((bsize,self.pred_len, height, width))
        # dec_out = torch.tensor(dec_out, device = self.device)
        # dec_out = dec_out.reshape((list(dec_out.shape[:-1]) + [num_lats, num_lons]   )) # (b, v, p, la, lo)
        return dec_out # (b, p, h,w) 
